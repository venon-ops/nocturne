...
export default function TicketsPage(){
 ...
 useEffect(()=>{let cancelled=false,timer:ReturnType<typeof setTimeout>|undefined,attempt=0;const returningFromCheckout=new URLSearchParams(location.search).get('success')==='1';async function load(){const supabase=getSupabase();const {data:{user}}=await supabase.auth.getUser();if(!user){location.assign('/auth?next=/tickets');return}const {data,error}=await supabase.from('tickets').select('id,qr_token,status,events(title,starts_at,city),ticket_types(name),orders!inner(buyer_id)').eq('orders.buyer_id',user.id).order('created_at',{ascending:false});if(cancelled)return;if(error){setError('Impossible de charger vos billets.');setLoading(false);return}const rows=(data??[]) as unknown as TicketRow[];setTickets(rows);const generated=await Promise.all(rows.map(async ticket=>[ticket.id,await QRCode.toDataURL(`nocturne:ticket:${ticket.qr_token}`,{width:240,margin:1})] as const));if(cancelled)return;setCodes(Object.fromEntries(generated));setLoading(false);if(returningFromCheckout&&!rows.length&&attempt++<10)timer=setTimeout(load,1500)}load();return()=>{cancelled=true;if(timer)clearTimeout(timer)}},[]);
