...
export default function EmbeddedCheckout({clientSecret,publishableKey,onClose}:{clientSecret:string;publishableKey:string;onClose:()=>void}){
 ...
 return <div className="checkout-overlay" role="dialog" aria-modal="true" aria-label="Paiement sécurisé"><section className="checkout-modal"><header><div><p className="eyebrow">PAIEMENT SÉCURISÉ</p><h2>Finaliser la commande</h2></div><button onClick={onClose} aria-label="Fermer"><X/></button></header>{error?<p className="profile-error">{error}</p>:<>{!ready&&<div className="checkout-loading"><LoaderCircle className="spin"/>Chargement de Stripe…</div>}<div ref={mountRef} className="embedded-checkout"/></>}</section></div>;
}
