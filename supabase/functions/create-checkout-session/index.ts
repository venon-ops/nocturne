...
Deno.serve(async req=>{
 ...
 try{
  ...
  const stripe=new Stripe(Deno.env.get('STRIPE_SECRET_KEY')!);const {data:org}=await db.from('organizer_profiles').select('stripe_account_id').eq('profile_id',firstEvent.organizer_id).single();
  ...
  const session=await stripe.checkout.sessions.create({mode:'payment',customer_email:user.email,line_items:items.map(item=>{const type:any=types.find(row=>row.id===item.ticketTypeId);return{price_data:{currency:'eur',unit_amount:type.price_cents,product_data:{name:`${firstEvent.title} — ${type.name}`}},quantity:item.quantity}}),success_url:Deno.env.get('APP_URL')!+'/tickets?success=1',cancel_url:Deno.env.get('APP_URL')!+'/events/'+firstEvent.slug,metadata:{items:JSON.stringify(items),buyerId:user.id,eventId:firstEvent.id},payment_intent_data:org?.stripe_account_id?{application_fee_amount:Math.round(total*.08),transfer_data:{destination:org.stripe_account_id}}:undefined});
