...
import { CalendarDays, Newspaper, Search, Ticket } from 'lucide-react';
...
export default function MobileHomeNav(){
 ...
 return <nav className="mobile-home-tabs" aria-label="Navigation principale mobile"><button className={view==='feed'?'active':''} onClick={()=>setView('feed')}><Newspaper/><span>Fil</span></button><button className={view==='upcoming'?'active':''} onClick={()=>setView('upcoming')}><CalendarDays/><span>À venir</span></button><button onClick={()=>location.assign('/tickets')}><Ticket/><span>Billets</span></button><button className={view==='search'?'active':''} aria-label="Recherche" onClick={()=>setView('search')}><Search/></button></nav>;
}
